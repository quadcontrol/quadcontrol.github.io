...
Previously, we [derived](../../fundamentals/mixer.md) the inverse mixer matrix $M^{-1}$, which converts total thrust and torques generated by the propellers ${\color{var(--c2)}f_t}$, ${\color{var(--c2)}\tau_x}$, ${\color{var(--c2)}\tau_y}$, and ${\color{var(--c2)}\tau_z}$ into the squared angular velocities of the four motors, $\omega_1$, $\omega_2$, $\omega_3$, and $\omega_4$:
...
We also [identified](../../identification/motor_coefficients.md) the motor coefficients $a_2$ and $a_1$, which convert the motor angular velocity $\omega$ into the corresponding PWM command:

$$
    {\color{var(--c3)}\text{PWM}} = a_2 {\color{var(--c1)}\omega}^2 + a_1 {\color{var(--c1)}\omega}
$$

Combining these two relationships gives us the complete mixer logic:

![](images/mixer.svg){: width="500" style="display: block; margin: auto;" }
...
Arm the quadcopter and vary the thrust command $f_t$ using the ++"Up"++ and ++"Down"++ buttons. Verify that all four motors increase and decrease their angular velocities accordingly.

![](images/thrust.svg){: width=60% style="display: block; margin: auto;" }

### Roll torque $\tau_x$

Arm the quadcopter and vary the roll torque $\tau_x$ using the ++"←"++ and ++"→"++ buttons. Verify that only motors 3 and 4 speed up for positive values, while motors 1 and 2 speed up for negative values.

![](images/roll.svg){: width=60% style="display: block; margin: auto;" }

### Pitch torque $\tau_y$

Arm the quadcopter and vary the pitch torque $\tau_y$ using the ++"↑"++ and ++"↓"++ buttons. Verify that only motors 2 and 3 speed up for positive values, while motors 1 and 4 speed up for negative values.

![](images/pitch.svg){: width=60% style="display: block; margin: auto;" }

### Yaw torque $\tau_z$

Arm the quadcopter and vary the yaw torque $\tau_z$ using the ++"←"++ and ++"→"++ buttons(1). Verify that only motors 2 and 4 speed up for positive values, while motors 1 and 3 speed up for negative values.
{ .annotate }
